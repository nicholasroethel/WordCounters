...
int main(int argc, char const *argv[]){	
	...
	int printwords = 0;
	int maxlen = 0;
	theWords *head = NULL; //list with all words
	theWords *head2 = NULL; //list with no reps
	theWords *head3 = NULL; //alpabetically ordered list with no reps
	frequencies *head4 = NULL; //list with frequencies
	char* tmpWord;
	int len = 0;
	FILE *input = NULL;
	...
	if(argc <3){
		...
		while(argv[count] != NULL){ //looks for arguments
			if(strcmp(argv[count],"--infile")==0){ //find the file
				input = fopen(argv[count+1], "r" ); //open the file
			}
			if(strcmp(argv[count],"--sort")==0){ //looks for sort
				sort = 1;
			}
			if(strcmp(argv[count],"--print-words")==0){ //looks for print-words
				printwords = 1;
			}
